...
par(mfrow = c(1,15),mar=c(1,0.5,1,0.5),oma=c(1,1,1,1))
#DO NOT change i
i = 7
while (i <= length(target_datExpr_trait[1,])) {
  ...
  #plot
  if(p_val <= 0.05 & my_cor_p <= 0.05){
    plot(x=target_datExpr_trait$temperature, y=target_datExpr_trait[,i],
         col = "#6699FF80", pch= 16, cex=2,
         main = NULL,
         xlab = "", ylab= "",
         xaxt = "n", yaxt = "n"
    )
    if(coef(fit)[2] > 0){
      abline(fit, col = "#FF333380", lty = 1, lwd = 10)
    }
  }
  if(p_val <= 0.05 & my_cor_p > 0.05){
    plot(x=target_datExpr_trait$temperature, y=target_datExpr_trait[,i],
    ...
    if(coef(fit)[2] > 0){
      abline(fit, col = "#FFA50080", lty = 1, lwd = 10)
    }
  }
  if(p_val > 0.05){
    plot(x=target_datExpr_trait$temperature, y=target_datExpr_trait[,i],
